//Extends the tree map which contains various methods 
//Uses a BalanceableBinaryTree which contains rotation functions
...
public class AVLTreeMap<K, V> extends TreeMap<K, V> {
	...
	protected Position<Entry<K, V>> tallerChild(Position<Entry<K, V>> p) {
		...
		if(isRoot(p)) {//In case p is root
			return left(p);
		}
		
		if(p == left(parent(p))) { //returns left child of p
			return left(p);
		}
	...
	protected void rebalance(Position<Entry<K, V>> p) {
		...
		//check if balanced and recompute heights
			old = height(p);
			if(!isBalanced(p)) {
			...
			//continue while height is changing
	  while (old != newest && p != null) {
		  old = height(p);
			if(!isBalanced(p)) {
				p = tree.restructure(tallerChild(tallerChild(p)));
				recomputeHeight(left(p));
