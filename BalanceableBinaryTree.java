public class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
	...
	public void rotate(Position<Entry<K, V>> p) {
		...
		if(x == y.getLeft()) { //rotates via relink based on position of x
	...
	public Position<Entry<K, V>> restructure(Position<Entry<K, V>> x) {
		...
		Node<Entry<K, V>> a = validate(x);
		Node<Entry<K, V>> b = a.getParent();
		Node<Entry<K, V>> c = b.getParent();
		
		if(c == null) { //if grandparent is null y is the root
			root = a;
			a.setParent(null);
		}else {
			relink(c,a,b == c.getLeft());
		}
		
		
		if((x == right(y)) && (y == right(z)) || (x == left(y)) && (y == left(z))) {
			rotate(y); //if one left or right rotate needed
			return y;
		}else {
			rotate(x); 
			rotate(x);
